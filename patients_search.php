...
// FIX : 'unsafe-inline' remplace 'strict-dynamic' pour autoriser les onclick=""
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_appt'])) {
        ...
        $stmt->bind_param("ii", $appt_id, $doc_id); $stmt->execute(); $stmt->close();
    ...
    if (isset($_POST['edit_appt'])) {
        ...
        $stmt->bind_param("sii", $new_date, $appt_id, $doc_id); $stmt->execute(); $stmt->close();
    ...
    if (isset($_POST['delete_patient'])) {
        ...
        $stmt->bind_param("is", $doc_id, $patient_name); $stmt->execute();
        $res = $stmt->get_result();
        $appt_ids = [];
        ...
        $stmt->bind_param("is", $doc_id, $patient_name); $stmt->execute(); $stmt->close();
...
$total_rdv_overall = 0; $total_avenir_overall = 0;
...
<head>
    ...
    <style nonce="<?= $nonce ?>">
        ...
        .patient-details { max-height: 0; overflow: hidden; transition: max-height 0.4s ease-out; }
        .patient-details.open { max-height: 9999px; }
    </style>
</head>
...
<div class="flex min-h-screen relative">
    ...
    <aside id="sidebar" class="w-64 bg-slate-900 border-r border-slate-800 flex flex-col fixed h-full z-50 transition-transform transform -translate-x-full lg:translate-x-0 print:hidden">
        ...
        <nav class="flex-1 p-4 space-y-1">
            ...
            <a href="chat_cabinet.php" class="sidebar-link flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium text-slate-400 hover:bg-slate-800 hover:text-white transition-colors">
                ...
                <span id="chat-badge-sidebar" class="hidden bg-rose-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">0</span>
            </a>
        </nav>
    ...
    <main class="flex-1 lg:ml-64 p-4 md:p-8 w-full">
        <div class="flex flex-wrap justify-between items-center mb-8 gap-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <p class="text-sm font-semibold text-slate-700 dark:text-slate-300 hidden sm:block"><?= date('d M, Y') ?></p>
            </div>
        </div>
        ...
        <form action="" method="GET" class="mb-8">
            <div class="flex gap-3">
                <input type="text" name="search" value="<?= htmlspecialchars($search_query) ?>"
                       ...
                       class="flex-1 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white">
                <button type="submit" class="bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition">Rechercher</button>
                <?php if ($search_query): ?>
                    <a href="patients_search.php" class="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-4 py-2.5 rounded-lg text-sm font-medium border border-slate-200 dark:border-slate-700 flex items-center">Effacer</a>
                <?php endif; ?>
            </div>
        </form>
                        ...
                        <div class="flex items-center gap-3">
                            <div class="hidden md:block text-right mr-2">
                                <p class="text-xs text-slate-500 dark:text-slate-400">
                                    Prochain RDV :
                                    <span class="font-semibold <?= $p['prochain_rdv'] ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400' ?>">
                            ...
                            <button onclick="openDeletePatient('<?= htmlspecialchars(addslashes($name), ENT_QUOTES) ?>', <?= count($appts) ?>)"
                                class="p-2 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 transition" title="Supprimer ce patient">
                    ...
                    <div id="<?= $pid ?>" class="patient-details border-t border-slate-100 dark:border-slate-800">
                        <div class="p-5 bg-slate-50 dark:bg-slate-800/30">
                            <div class="flex justify-between items-center mb-3">
                                <h4 class="text-xs font-bold uppercase text-slate-500 dark:text-slate-400 tracking-wider">
                                    Historique (<?= count($appts) ?> séance<?= count($appts) > 1 ? 's' : '' ?>)
                                </h4>
                                <a href="consultations.php?patient_name=<?= urlencode($name) ?>" class="text-xs text-indigo-600 dark:text-indigo-400 font-medium hover:underline">
                                    Toutes les archives →
                                </a>
                            </div>

                            <?php if(empty($appts)): ?>
                                <p class="text-xs text-slate-400 italic text-center py-4">Aucune séance enregistrée.</p>
                            <?php else: ?>
                            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-100 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-700">
                                ...
                                <div class="flex items-center justify-between px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="text-center px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 min-w-[40px]">
                                            <span class="block text-slate-500 dark:text-slate-400 text-[10px] uppercase font-bold"><?= date('M', $ts) ?></span>
                                            <span class="block text-slate-800 dark:text-slate-200 font-bold text-sm leading-tight"><?= date('d', $ts) ?></span>
                                        </div>
                                        <div>
                                            <p class="text-sm text-slate-700 dark:text-slate-300 font-medium">
                                                <?= date('H:i', $ts) ?> · <?= date('Y', $ts) ?>
                                                <?php if ($isArch): ?>
                                                ...
                                                <?php else: ?>
                                                    <span class="ml-2 text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded uppercase font-bold">Passé</span>
                                                <?php endif; ?>
                                            </p>
                                        </div>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <?php if (!$isArch): ?>
                                            <button onclick="openEdit(<?= $appt['id'] ?>, '<?= date('Y-m-d\TH:i', $ts) ?>', '<?= htmlspecialchars(addslashes($name), ENT_QUOTES) ?>')"
                                                class="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition" title="Modifier">
                                            ...
                                            <button onclick="openConfirm(<?= $appt['id'] ?>, '<?= htmlspecialchars(addslashes($name), ENT_QUOTES) ?>')"
                                                class="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 rounded text-slate-400 hover:text-red-600 transition" title="Supprimer RDV">
...
<div id="edit-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden" onclick="if(event.target===this)closeEdit()">
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-md border dark:border-slate-700" onclick="event.stopPropagation()">
        ...
        <form action="" method="POST" class="p-6 space-y-4">
            ...
            <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg font-bold text-sm">Sauvegarder</button>
        </form>
    </div>
</div>
...
<div id="confirm-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden" onclick="if(event.target===this)closeConfirm()">
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-sm border dark:border-slate-700" onclick="event.stopPropagation()">
        <div class="p-6 text-center">
            <div class="w-12 h-12 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4 text-red-600">
            ...
            <form action="" method="POST" class="flex gap-3">
                ...
                <input type="hidden" name="back_search" value="<?= htmlspecialchars($search_query) ?>">
                <button type="button" onclick="closeConfirm()" class="flex-1 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-medium text-sm">Annuler</button>
                <button type="submit" class="flex-1 py-2 rounded-lg bg-red-600 text-white font-medium text-sm">Supprimer</button>
            </form>
        </div>
    </div>
</div>
...
<div id="delete-patient-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden" onclick="if(event.target===this)closeDeletePatient()">
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-sm border dark:border-slate-700" onclick="event.stopPropagation()">
        <div class="p-6 text-center">
            <div class="w-14 h-14 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4 text-red-600">
            ...
            <p class="text-xs text-red-500 font-medium mb-6">⚠️ Action irréversible — RDV et archives supprimés.</p>
            <form action="" method="POST" class="flex gap-3">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'], ENT_QUOTES, 'UTF-8') ?>">
                <input type="hidden" name="delete_patient" value="1">
                <input type="hidden" name="patient_name" id="delete-patient-input">
                <button type="button" onclick="closeDeletePatient()" class="flex-1 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-medium text-sm">Annuler</button>
                <button type="submit" class="flex-1 py-2.5 rounded-lg bg-red-600 text-white font-bold text-sm">Supprimer</button>
            </form>
        </div>
    </div>
</div>
