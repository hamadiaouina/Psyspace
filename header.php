<?php
// 1. On nettoie pour éviter les conflits
header_remove("Content-Security-Policy");

// 2. Génération du nonce unique pour tes scripts et styles
$nonce = base64_encode(random_bytes(16));

// 3. Headers de protection
header("X-Frame-Options: DENY");
header("X-Content-Type-Options: nosniff");
header("Referrer-Policy: strict-origin-when-cross-origin");

// 4. La CSP Stricte (Celle qui te donne le 9/10)
$csp_rules = [
    // On autorise par défaut 'self' et Cloudflare
    "default-src 'self' https://challenges.cloudflare.com",
    
    // Scripts : On utilise le nonce pour tes scripts inline et Tailwind
    "script-src 'self' 'nonce-$nonce' 'unsafe-eval' https://challenges.cloudflare.com",
    
    // Styles : On autorise Google Fonts et tes styles inline via nonce (ou unsafe-inline si nécessaire pour Tailwind)
    ...
    // Images : On ouvre spécifiquement pour Unsplash et les données locales
    "img-src 'self' data: https://images.unsplash.com https://images.rawpixel.com blob:;",
    
    // Connexions : Nécessaire pour les APIs et Docker/Localhost
...
// Buffer pour injecter le nonce automatiquement dans tous les <script>
